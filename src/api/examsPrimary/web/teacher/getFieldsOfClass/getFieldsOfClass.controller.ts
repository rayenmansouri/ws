...
export class GetFieldsOfClassController extends BaseController<GetFieldsOfClassRouteConfig> {
  constructor(
    @inject("GetFieldsOfClassUseCase")
    private getFieldsOfClassUseCase: GetFieldsOfClassUseCase,
  ) {
    super();
  }

  async main(req: TypedRequest<GetFieldsOfClassRouteConfig>): Promise<void | APIResponse> {
    const result = await this.getFieldsOfClassUseCase.execute({
    ...
    return new SuccessResponse<GetFieldsOfClassResponse>("global.success", result);
  }
}
