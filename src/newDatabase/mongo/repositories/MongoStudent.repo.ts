...
export class MongoStudentRepo extends MongoBaseRepo<StudentMetaData> implements StudentRepo {
  ...
  async findUnarchiveStudentByNewIdOrThrow<
  ...
  >(
    ...
    const query: FilterQuery<Student> = { newId: studentNewId, isArchived: false };

    const queryResult = this.model.findOne(query);

    if (option?.populate) queryResult.populate(option.populate as string[]);

    const student = await queryResult.lean();

    if (!student) throw new Error(`Student with newId ${studentNewId} not found or is archived.`);

    return student as Populate<StudentMetaData, FieldsToPopulate>;
  }
