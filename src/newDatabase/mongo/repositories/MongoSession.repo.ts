...
import {
  getLatestAttendanceForClassesDto,
...
export class MongoSessionRepo extends MongoBaseRepo<SessionMetaData> implements SessionRepo {
  ...
  async getStudentSchedule(
    ...
    const query: FilterQuery<Session> = {
      $or: [
        { class: classId, classGroup: classGroupId },
        { class: classId, classGroup: null },
      ],
      startTime: { $gte: range.startDate },
      endTime: { $lte: range.endDate },
    };
  ...
  async getLatestAttendanceForClasses(
    classIds: ID[],
    dateInterval?: { from?: Date; to?: Date },
  ): Promise<getLatestAttendanceForClassesDto[]> {
    const timeQuery: { $gte?: Date; $lte?: Date } = {};
    if (dateInterval) {
      if (dateInterval.from) {
        timeQuery.$gte = dateInterval.from;
    ...
    const attendancePipeline: PipelineStage[] = [
      {
        $match: {
          class: { $in: stringsToObjectIds(classIds) },
          ...(dateInterval ? { startTime: timeQuery } : {}),
          attendence: { $ne: {} },
        },
      },
    ...
    const response = await this.model.aggregate<getLatestAttendanceForClassesDto>(
      attendancePipeline,
    );
