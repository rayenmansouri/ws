...
@injectable()
export class PublishTermUseCase {
  constructor(
  ...
  async execute(classNewId: string, termNewId: string): Promise<void> {
    ...
    if (termGradeReport.isPublished) throw new BadRequestError("term.alreadyPublished");

    await this.classRepo.publishTerm(classDoc._id, termDoc._id);
  }
}
