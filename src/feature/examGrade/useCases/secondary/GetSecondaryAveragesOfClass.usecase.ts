...
import { SecondaryAveragesOfClassDTO } from "../../dto/secondary/SecondaryAveragesOfClass.dto";

@injectable()
export class GetSecondaryAveragesOfClassUsecase {
  constructor(
  ...
  async execute(classNewId: string, termNewId: string): Promise<SecondaryAveragesOfClassDTO> {
    ...
    const classesGrades = await Promise.all(
      classes.map(currentClass =>
        this.secondaryClassGradesRepo.loadTermClassGrades(currentClass._id, termDoc._id),
      ),
    );
    ...
    const classAverages: SecondaryAveragesOfClassDTO["classAverages"] = classesGrades
      .map(currentClassGrades => {
        return {
          className: currentClassGrades.class.name,
          classNewId: currentClassGrades.class.newId,
          rank: null,
    ...
    const studentAverages: SecondaryAveragesOfClassDTO["studentAverages"] = classDoc.students
      .map(student => {
