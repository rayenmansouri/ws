...
import { PrimaryAveragesOfClassDTO } from "../../dto/primary/PrimaryAveragesOfClass.dto";
...
@injectable()
export class GetPrimaryAveragesOfClassUsecase {
  constructor(
  ...
  async execute(classNewId: string, termNewId: string): Promise<PrimaryAveragesOfClassDTO> {
    ...
    const classesGrades = await Promise.all(
      classes.map(currentClass =>
        this.primaryClassGradesRepo.loadTermClassGrades(currentClass._id, termDoc._id),
      ),
    );
    ...
    const classAverages: PrimaryAveragesOfClassDTO["classAverages"] = classesGrades
    ...
    const studentAverages: PrimaryAveragesOfClassDTO["studentAverages"] = classDoc.students
      .map(student => {
