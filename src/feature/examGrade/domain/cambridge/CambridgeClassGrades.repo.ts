...
import { CambridgeAnnualClassGrades } from "./CambridgeAnnualClassGrades.entity";
...
@injectable()
export class CambridgeClassGradesRepo {
  constructor(
  ...
  async loadTermClassGrades(classId: ID, termId: ID): Promise<CambridgeTermClassGrades> {
    ...
    const subjects: CambridgeGradingEntity[] = [];
    ...
    for (const subject of classType.subjects) {
      ...
      const subjectExamGrades = examGrades.map(examGrade => {
        ...
        return new CambridgeExamGrades({
          ...
          studentGrades: Object.entries(examGrade.degrees).map(([studentId, degree]) => {
            return {
              ...
              grade: CambridgeGrade.create(examCoefficient, degree),
            };
          }),
        });
      });
    ...
    const groupGrades: CambridgeGradingEntity[] = [];
    ...
    const adminObservations = allGradeBookObservations.find(
      observations => observations.topicId === null && observations.topicType === null,
    );
    if (!adminObservations) throw new InternalError();

    const classGrades = new CambridgeTermClassGrades({
  ...
  async loadAnnualClassGrades(classId: ID, termIds: ID[]): Promise<CambridgeAnnualClassGrades> {
    const termClassGrades = await Promise.all(
      termIds.map(termId => this.loadTermClassGrades(classId, termId)),
    );

    return new CambridgeAnnualClassGrades(termClassGrades);
  }

  async loadGroupGrades(groupId: ID, termId: ID): Promise<CambridgeGradingEntity> {
    ...
    return new CambridgeGradingEntity({
      name: group.groupType.name,
      newId: group.newId,
      coefficient,
      studentIds: group.students,
      ...
      examGrades: group.groupType.exams.map(exam => {
        ...
        return new CambridgeExamGrades({
          ...
          studentGrades: Object.entries(examGrade.degrees).map(([studentId, degree]) => {
            return {
              studentId: studentId as ID,
              grade: CambridgeGrade.create(exam.coefficient, degree),
            };
          }),
        });
      }),
